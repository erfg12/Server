...
EQApplicationPacket* ServerManager::CreateTrilogyServerListPacket(Client* c)
{

	unsigned int packet_size = sizeof(TrilogyServerList_Struct);
	...
	TrilogyServerList_Struct *sl = (TrilogyServerList_Struct*)outapp->pBuffer;
	sl->numservers = server_count;
	//sl->showusercount = 0;

	unsigned char *data_ptr = outapp->pBuffer;
	data_ptr += sizeof(TrilogyServerList_Struct);
